...
(defclass track ()
  ((title :reader title :initform nil)
   ...
   (number :reader track-number :initform nil)
...
(defclass relation ()
  ((type :reader relation-type :initform nil)
...
(defun make-relation (type target &key direction attributes begin end)
  (macrolet ((set-slot (slot-name)
             `(setf (slot-value rel ',slot-name) ,slot-name))
             (maybe-set-slot (slot-name)
               `(when ,slot-name (set-slot ,slot-name))))
    (let ((rel (make-instance 'relation)))
      (set-slot type) (set-slot target)
      (setf (slot-value rel 'target-id) (id target))
      (maybe-set-slot direction) (maybe-set-slot attributes)
      (maybe-set-slot begin) (maybe-set-slot end)
      rel)))
